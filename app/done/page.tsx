...
export default function DonePage() {
  ...
  useEffect(() => {
    ...
    setHasGrade(!!localStorage.getItem('gradeResult'))
    setOverloaded(localStorage.getItem('gradeOverloaded') === 'true')
  }, [])
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="max-w-lg w-full text-center fade-in">
        ...
        {/* AI Grading CTA */}
        {overloaded ? (
          <div className="rounded-2xl p-5 mb-4 text-left bg-amber-50 border border-amber-200">
            <div className="flex items-start gap-3">
              <span className="text-3xl">⏳</span>
              <div className="flex-1">
                <h4 className="font-bold text-amber-800 mb-1">Hệ thống đang quá tải</h4>
                <p className="text-amber-700 text-sm leading-relaxed mb-2">
                  Chúng tôi sẽ gửi kết quả chấm điểm cho bạn sau nhé. Xin cảm ơn!
                </p>
                {candidate?.email && (
                  <p className="text-amber-600 text-xs">
                    📧 Kết quả sẽ được gửi về: <strong>{candidate.email}</strong>
                  </p>
                )}
              </div>
            </div>
          ...
          <div className={`rounded-2xl p-5 mb-4 text-left ${hasGrade ? 'bg-gradient-to-r from-indigo-600 to-blue-600' : 'bg-gray-100 border border-gray-200'}`}>
            <div className="flex items-start gap-3">
              <span className="text-3xl">🤖</span>
              <div className="flex-1">
                <h4 className={`font-bold mb-1 ${hasGrade ? 'text-white' : 'text-gray-600'}`}>
                  Kết quả AI chấm điểm
                </h4>
                {hasGrade ? (
                  <>
                    <p className="text-indigo-200 text-sm mb-3">
                      Gemini đã chấm xong! Xem điểm số và nhận xét chi tiết từng part.
                    </p>
                    <button
                      onClick={() => router.push('/result')}
                      className="bg-white text-indigo-700 font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-indigo-50 transition shadow"
                    >
                      Xem kết quả ngay →
                    </button>
                  </>
                  ...
                  <p className="text-gray-500 text-sm">
                    Chưa có kết quả — kiểm tra GEMINI_API_KEY trong .env.local
                  </p>
                )}
              </div>
            </div>
