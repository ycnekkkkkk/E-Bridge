...
export default function TestPage() {
  ...
  const gradeWithAI = useCallback(async () => {
    try {
      const fd = new FormData()
      fd.append('folderId', folderIdRef.current || '')
      fd.append('writing', writingRef.current)
      fd.append('candidateInfo', localStorage.getItem('candidateInfo') || '{}')
      for (const [id, blob] of Object.entries(audioBlobsRef.current)) {
        fd.append(`audio_${id}`, blob, `part${id}.webm`)
      }
      const res = await fetch('/api/grade', { method: 'POST', body: fd })
      const data = await res.json()
      if (data.success) {
        localStorage.setItem('gradeResult', JSON.stringify(data.grades))
        localStorage.removeItem('gradeOverloaded')
      } else if (data.overloaded) {
        localStorage.setItem('gradeOverloaded', 'true')
  ...
  const handleNext = useCallback(() => {
    ...
    if (partIndex < PARTS.length - 1) {
      ...
      // Last part done — send audio to Gemini for grading
      setPhase('grading')
      gradeWithAI()
    }
  }, [partIndex, clearTimer, gradeWithAI])
      ...
      <header className="bg-white border-b border-indigo-100 shadow-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-indigo-900 text-sm">E-Bridge Test</span>
          </div>
          {candidate && (
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-6">
        ...
        {phase === 'grading' && (
          <div className="fade-in flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-2xl font-bold text-indigo-900 mb-2">Hệ thống đang chấm điểm...</h3>
            <p className="text-gray-500 mb-1">Hệ thống đang nghe và phân tích toàn bộ bài thi của bạn</p>
            <p className="text-xs text-gray-400">Thường mất 15–30 giây, vui lòng không tắt trang</p>
