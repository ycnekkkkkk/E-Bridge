...
async function loadRobotoFont(): Promise<Buffer> {
  if (fontBuffer) return fontBuffer
  const url = 'https://cdn.jsdelivr.net/gh/google/fonts@main/apache/roboto/Roboto-Regular.ttf'
  return new Promise((resolve, reject) => {
    https.get(url, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        const redirectUrl = res.headers.location!
        const getter = redirectUrl.startsWith('https') ? https : http
        getter.get(redirectUrl, (r2) => {
          const chunks: Buffer[] = []
          r2.on('data', (c: Buffer) => chunks.push(c))
          r2.on('end', () => { fontBuffer = Buffer.concat(chunks); resolve(fontBuffer) })
          r2.on('error', reject)
        })
      } else {
        const chunks: Buffer[] = []
        res.on('data', (c: Buffer) => chunks.push(c))
        res.on('end', () => { fontBuffer = Buffer.concat(chunks); resolve(fontBuffer) })
        res.on('error', reject)
      }
    }).on('error', reject)
  })
